...
void app_hp_jack_init(void)
{
	...
	if (HP_JACK_DET < _PB_0) {
		...
		InterruptRegister(GPIO_INTHandler, GPIOB_IRQ, (u32)GPIOB_BASE, 10);
...
void app_captouch_init(void)
{
	...
	InterruptRegister((IRQ_FUN) app_captouch_irq_handler, CTOUCH_IRQ, (u32)NULL, 10);
	InterruptEn(CTOUCH_IRQ, 10);
...
void app_keyscan_init(u8 reset_status)
{
	...
	if(FALSE == reset_status) {
		...
		InterruptRegister((IRQ_FUN) app_keyscan_irq_handler, KEYSCAN_IRQ, NULL, 10);
		InterruptEn(KEYSCAN_IRQ, 10);
	}
}
