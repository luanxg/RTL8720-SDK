 ...
 u32 SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
	...
	/***** CMD32 *****/
	cmd_attr.arg = start;
	cmd_attr.idx = SD_CMD_EraseBlkSt;
	cmd_attr.rsp_type = SDIOH_RSP_6B;
	cmd_attr.rsp_crc_chk = ENABLE;
	cmd_attr.data_present = SDIOH_NO_DATA;
	ret = SDIOH_SendCommand(&cmd_attr, SDIOH_CMD_CPLT_TIMEOUT);
	...
	ret = CmdRespError(SDIOH_RESP_R1, SD_CMD_EraseBlkSt);
	if (ret != HAL_OK) {
		return ret;
	}
	...
	/***** CMD33 *****/
	cmd_attr.arg = end;
	cmd_attr.idx = SD_CMD_EraseBlkEd;
	cmd_attr.rsp_type = SDIOH_RSP_6B;
	cmd_attr.rsp_crc_chk = ENABLE;
	cmd_attr.data_present = SDIOH_NO_DATA;
	ret = SDIOH_SendCommand(&cmd_attr, SDIOH_CMD_CPLT_TIMEOUT);
	...
	ret = CmdRespError(SDIOH_RESP_R1, SD_CMD_EraseBlkEd);
	if (ret != HAL_OK) {
		return ret;
	}
...
SD_RESULT SD_Init(void)
{
	...
	InterruptRegister((IRQ_FUN)SD_IRQHandler, SDIO_HOST_IRQ, NULL, 10);
	InterruptEn(SDIO_HOST_IRQ, 10);
